 ...
 * 作者：杨友能
 */
class BackstagesBaseView {
	protected $smarty		= '';
	protected $page			= '';
	protected $loginAdminName	= '';
	protected $loginAdminEmail	= '';
	...
	public  function __construct(){
		...
		$this->smarty->template_dir = WEB_PATH . 'html/template/backstagestemplate' . DIRECTORY_SEPARATOR; //模板文件目录
		...
		if(C("BACK_IS_AUTH_ON") === true){
			if(self::checkModuleAuth($mod, $act)){
				if(!isset($_SESSION['admin_name']) || empty($_SESSION['admin_id'])){
					...
					$this->loginAdminEmail	= isset($_SESSION['admin_email'])	? $_SESSION['admin_email']	: '';
					$this->loginAdminID		= isset($_SESSION['admin_id'])		? $_SESSION['admin_id']		: 0;
					$this->loginAdminName	= isset($_SESSION['admin_name'])		? $_SESSION['admin_name']	: '';
				}
			}
		}
		//设置get post提交过来的数据
		...
		$this->smarty->assign("login_admin_name", $this->loginAdminName);
		$this->smarty->assign('login_admin_id', $this->loginAdminID);
		$this->smarty->assign('login_admin_email', $this->loginAdminEmail);
	}
	...
	protected function checkModuleAuth($mod, $act){
		$arrModule	= explode(',', C("BACK_IS_AUTH_MODULE"));
